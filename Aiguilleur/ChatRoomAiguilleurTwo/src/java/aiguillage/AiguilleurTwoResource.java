...
@Singleton
@Path("AiguilleurTwo")
public class AiguilleurTwoResource {

    @Context
    private UriInfo context;
    private WebTarget webTarget;
    private WebTarget OneTarget;
    private Client client;
    private String AiguilleurOneURI = "http://localhost:8080/ChatRoomAiguilleurOne/aiguillage/AiguilleurOne";
    ...
    public String register(String pseudo){
        ...
        if (this.users.contains(pseudo)) {
            ...
            this.client = ClientBuilder.newClient();
            Response test = this.client.target(this.AiguilleurOneURI).path("test").request(MediaType.TEXT_PLAIN).get();
            if (test.getStatus() == 200){
                String response = client.target(this.AiguilleurOneURI).path("getallusers").request(MediaType.TEXT_PLAIN).get(String.class);
    ...
    public void postMessage(@PathParam("idChatRoom") String idChatRoom, String message){
        ...
        /* envoie du msg a AiguilleurOne s'il est UP */
         Response result = client.target(this.AiguilleurOneURI).path("test").request(MediaType.TEXT_PLAIN).get();
        if (result.getStatus() == 200){
            client.target(this.AiguilleurOneURI).path("postbridge").path("{idChatRoom}").resolveTemplate("idChatRoom", idChatRoom)
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    public void postByOne(@PathParam("idChatRoom") String idChatRoom, String message){
        OutboundEvent.Builder eventBuilder = new OutboundEvent.Builder();
        OutboundEvent event = eventBuilder.name(idChatRoom)
            .mediaType(MediaType.TEXT_PLAIN_TYPE)
            .data(String.class, message)
